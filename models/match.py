...
class ScoreBreakdown(BaseModel):
    role_fit: float = Field(
        ge=0,
        le=100,
        description="Does the role match target_titles + target_role_families?",
    )
    ...
    seniority_match: float = Field(
        ge=0,
        le=100,
        description="Seniority alignment (handles both IC and management tracks)",
    )
...
class JobMatch(BaseModel):
    ...
    red_flags_hit: list[str] = Field(
        description="Deal-breakers or red flags found in JD"
    )
